...
# 2. Extract Text and Arrows - Smooth Alpha
...
for y in range(h):
    for x in range(w):
        if solid_map_mask[y, x] == 0:
            pixel_gray = gray[y, x]
            if pixel_gray < 185:
                # Smooth fade from Solid (150) to Transparent (185)
...
# 5. UNIFORM PURE BLACK TEXT FIX (The True Anti-Fringe Fix)
original_text_zone = (final_map_alpha == 0) & (text_alpha > 0)

# 5a. OUTSIDE MAP: EVERYTHING in the text zone is forced to EXACTLY (0,0,0) RGB!
# This keeps the transparent alpha values intact (for smoothness) but guarantees
# the RGB color is never beige, so there is no color "coating" against light backgrounds.
final_img[original_text_zone, 0] = 0 # B
final_img[original_text_zone, 1] = 0 # G
final_img[original_text_zone, 2] = 0 # R

# 5b. INSIDE MAP: Arrow Bases
# We geographically expand the text zone inwards to find arrow bases touching the map.
...
arrow_bases_inside = (final_map_alpha > 0) & (dilated_text_zone == 1) & (gray < 185) & (hsv[:,:,1] < 45)

final_img[arrow_bases_inside, 0] = 0 # B
final_img[arrow_bases_inside, 1] = 0 # G
final_img[arrow_bases_inside, 2] = 0 # R

cv2.imwrite(out_path, final_img)
print(f"Algorithm 16 (Anti-Aliased Pure Black) complete. Saved seamlessly to: {out_path}")
