...
# The text zone was previously only colored black OUTSIDE the map.
original_text_zone = (final_map_alpha == 0) & (text_alpha > 0)

# We geographically expand the "text coloring zone" 40 pixels deep into the map inwards.
# This strictly captures the "bases" of the arrows where they attach to the gray map.
...
# Inside this expanded text zone, anything that is dark (original gray < 130) is forced to pure black (0,0,0).
# This perfectly unifies the color of the arrow tip (which was black) and the arrow base (which was gray).
arrow_bases_and_text = (dilated_text_zone == 1) & (gray < 130) & (final_alpha > 0)
final_img[arrow_bases_and_text, 0] = 0 # B
final_img[arrow_bases_and_text, 1] = 0 # G
final_img[arrow_bases_and_text, 2] = 0 # R

cv2.imwrite(out_path, final_img)
print(f"Algorithm 13 (Uniform Black Text) complete. Saved seamlessly to: {out_path}")
